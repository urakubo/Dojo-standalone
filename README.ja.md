...
### 2D/3Dフィルタ
2Dスタック画像を対象とした、多数の2D/3Dフィルタ関数が実装されています。順次機能拡張予定です。
上端のプルダウンメニュー左から四番目のPlugins → 2D Filters, 2D Watershed, 3D Filtersを選択してください。各フィルタのダイアログが起動します。特に、2D DNN適用後セグメンテーションの後処理に利用することを想定しています。
