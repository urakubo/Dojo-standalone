...
深層学習のプログラム知識を持たない実験研究者が、EM画像/正解セグメンテーションをもとに「セグメンテーション」「人手による校正（プルーフリード）」までできるようにすることを目標とします。正解セグメンテーションの作成はVast liteの使用をお勧めします ( https://software.rc.fas.harvard.edu/lichtman/vast/ ) 。
...
上端のプルダウンメニュー左から四番目のPlugins → 2D Filters, 2D Watershed, 3D Filtersを選択してください。各フィルタのダイアログが起動します。特に、2D DNN適用後セグメンテーションの後処理に利用することを想定しています。順次機能拡張予定です。
